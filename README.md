...
백준(BOJ), SWEA(Samsung SW Expert Academy), 프로그래머스(Programmers), LeetCode에서 정답 제출 시 코드를 수집해 Notion 데이터베이스에 저장하는 Chrome 확장입니다.
...
1. 문제 풀이 페이지(`leetcode.com/problems/`)에서 **MutationObserver**로 `Accepted` 결과 감지
2. Analysis 버튼 옆에 **업로드** 버튼 자동 주입 (Accepted 시에만)
3. Monaco Editor 코드를 MAIN world 브릿지(`programmers_monaco_bridge.js`)로 추출
4. 백엔드 서버(`/analyze`)로 전송 → AI 분석 후 Notion에 저장

> 난이도(Easy/Medium/Hard), 런타임, 메모리가 자동 추출되어 함께 저장됩니다.  
> 문제 설명도 README에 자동 기록됩니다.
...
│   ├── leetcode_content.js          # LeetCode Accepted 감지 + 업로드 버튼 주입
│   └── programmers_monaco_bridge.js # MAIN world Monaco 에디터 코드 추출 (프로그래머스·LeetCode 공용)
├── options/
...
└── scripts/
    ...
    ├── language_normalizer.js  # 언어명 정규화 (백준 + SWEA + 프로그래머스 + LeetCode)
    ├── notion_client.js        # Notion API 직접 호출 (페이지 생성)
    ├── oauth.js                # Notion OAuth 흐름 (연결/해제/토큰 조회/DB 목록 조회)
    ├── payload_builder.js      # /analyze 페이로드 조립 (백준 + SWEA + 프로그래머스 + LeetCode)
...
1. LeetCode에서 문제 풀이 후 **Submit** 클릭
2. **Accepted** 결과가 뜨면 Analysis 버튼 옆 **업로드** 버튼 클릭
3. Notion 데이터베이스에 자동 저장

## 업데이트 이력

### v2.7.0
- **LeetCode 플랫폼 지원 추가** — `leetcode.com/problems/` Accepted 제출을 Notion·GitHub에 자동 저장
  - MutationObserver로 `Accepted` 결과 감지 → Analysis 버튼 옆 업로드 버튼 자동 주입
  - 난이도(Easy/Medium/Hard), 런타임(ms), 메모리(MB) 자동 추출
  - Monaco Editor 코드 추출에 `programmers_monaco_bridge.js` 재활용 (MAIN world CSP 우회)
  - 문제 설명(`data-track-load="description_content"`) 스크래핑 → GitHub README에 기록
  - SPA 네비게이션 대응 — URL 변경 감지로 새 문제 이동 시 버튼 재주입
- **AI 분석 태그 23개 고정 카테고리로 제한** — array, dynamic-programming, bfs 등 허용 목록 내에서만 반환
- **태그 README 반영** — GitHub 커밋 시 `analysisResult.tags`가 README 분류 섹션에 기록
...
| AlgoNotion-Platform | LeetCode 플랫폼 지원 추가 (v2.7.0) | ssong |
| AlgoNotion-LeetCode-Test | LeetCode 연동 QA 테스트 22/22 통과 | ssong |
